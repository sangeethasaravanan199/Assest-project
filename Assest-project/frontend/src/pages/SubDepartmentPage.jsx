...
const STORAGE_KEY = "asset-sub-departments";
const DEPT_STORAGE_KEY = "asset-departments";

const defaultSubDepartments = [
  { id: 1, name: "Network & Security", department: "Information Technology", description: "Handles network infrastructure and cybersecurity" },
  { id: 2, name: "Software Development", department: "Information Technology", description: "Product development and maintenance" },
  { id: 3, name: "Recruitment", department: "Human Resources", description: "Talent acquisition and onboarding" },
  { id: 4, name: "Payroll", department: "Human Resources", description: "Salary processing and benefits" },
  { id: 5, name: "Accounts Payable", department: "Finance", description: "Vendor payments and invoices" },
];

const defaultDepartments = [
  "Information Technology", "Human Resources", "Finance", "Operations", "Marketing",
];
...
export default function SubDepartmentPage() {
  const [rows, setRows] = useState(() => {
    try {
      ...
      return saved ? JSON.parse(saved) : defaultSubDepartments;
    } catch {
      return defaultSubDepartments;
    }
  });
  ...
  const openEdit = (row) => {
    setEditRow(row);
    setForm({ name: row.name, department: row.department, description: row.description });
    setDialogOpen(true);
  };
  ...
  const columns = [
    ...
    { field: "name", headerName: "Sub Department Name", flex: 1, minWidth: 200 },
    {
      field: "department",
  ...
  const summaryCards = [
    { label: "Total Sub Departments", value: rows.length, icon: <LayersOutlinedIcon />, color: "#0f766e" },
    { label: "Linked Departments", value: uniqueDepts, icon: <AccountTreeOutlinedIcon />, color: "#155fa0" },
  ];
  ...
  return (
    <Stack spacing={2.5}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, ml: "auto", width: "100%", maxWidth: 600 }}>
          <TextField
            size="small"
        ...
        <Stack sx={{ height: "100%" }}>
          <Box sx={{ p: 2.5, borderBottom: "1px solid", borderColor: "divider", bgcolor: "#f8fafc" }}>
            <Typography sx={{ fontWeight: 700, color: "#0f766e", fontSize: 28, pb: 0.5 }}>
              {editRow ? "Edit Sub Department" : "Add Sub Department"}
            </Typography>
          </Box>
